...
import { appendSingleUser, appendUserAndSortPost } from "../utils/post.utils";
...
export class PostController {
  ...
  public static getAllPosts = async (req: Request, res: Response) => {
    try {
      ...
      return ResponseController.HandleSuccessResponse(res, {
        status: 200,
        message: "Posts found!",
        data: appendUserAndSortPost(posts, user),
      });
    } catch (error) {
      return ResponseController.Handle500Error(res, error);
    }
   ...
   */
  public static getRelevantPosts = async (req: Request, res: Response) => {
    const totalPosts = Number(req.query.total as string) || 10;
    const followingPostsPercentage = 0.9;
    ...
    try {
      ...
      const followingPostsCount = Math.floor(
        totalPosts * followingPostsPercentage
      );

      const followingPosts = await Post.aggregate([
        { $match: { createdBy: { $in: user.following } } },
        { $sample: { size: followingPostsCount } },
      ]);

      const randomPosts = await Post.aggregate([
        { $match: { _id: { $nin: [...followingPosts] } } },
        { $sample: { size: totalPosts - followingPosts.length } },
      ]);

      const posts = [...followingPosts, ...randomPosts];
      // append user and sort posts
      const newPosts = await Promise.all(
        posts.map(async (post) => {
          const user = await User.findOne({ _id: post.createdBy });
          return appendSingleUser(post, user);
        })
      );
